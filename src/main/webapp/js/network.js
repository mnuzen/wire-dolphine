    ...
    */
    function populateGraph(){
      ...
      for (var i = 1; i < data.length; i++) {
        ...
        if (ip == 17){ 
          nodes[counter] = { id: counter, label: data[i].destination, group: GROUP1};

          // add edges based on freqs
          for (var j = 0; j < data[i].frequency; j++) {
            edges[ed] = {from: counter, to: GROUP1};
            ed++;
          } 
        }
        else if (ip == 18) { 
          nodes[counter] = { id: counter, label: data[i].destination, group: GROUP2};

          // add edges based on freqs
          for (var j = 0; j < data[i].frequency; j++) {
            edges[ed] = {from: counter, to: GROUP2};
            ed++;
          }
        } 
        else {
